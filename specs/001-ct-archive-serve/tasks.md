...
- [ ] T008 Wire HTTP handler skeleton, metrics endpoint, and Content-Type mapping in `internal/ct-archive-serve/server.go` (serve `GET /metrics` for Prometheus; set appropriate `Content-Type`: `*.json` → `application/json`, `/checkpoint` → `text/plain; charset=utf-8`, tiles → `application/octet-stream`, issuers → `application/pkix-cert`; return `404` on missing content)
...
- [ ] T013 [P] [US1] Add monitor.json config env var in `internal/ct-archive-serve/config.go` (`CT_MONITOR_JSON_REFRESH_INTERVAL`)
...
- [ ] T021 [P] [US2] Implement tile index decoding (tlog-style `xNNN/NNN`) in `internal/ct-archive-serve/routing.go`
...
- [ ] T031 [P] Add performance tuning env vars in `internal/ct-archive-serve/config.go` (`CT_ZIP_CACHE_MAX_OPEN`, `CT_ARCHIVE_REFRESH_INTERVAL`) and document defaults per `spec.md` (`FR-011`)
