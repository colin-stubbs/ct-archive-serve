...
- **Refresh concurrency protection**: Both `MonitorJSONBuilder` and `ArchiveIndex` use mutex protection to serialize refresh operations, preventing concurrent refreshes that could waste resources (e.g., opening the same ZIP files multiple times, scanning disk concurrently).
- **ZIP optimization for monitor.json**: When building monitor.json snapshots, each `000.zip` is opened only once per log to extract both `log.v3.json` and check for `issuer/` entries, rather than opening the same ZIP file twice. This significantly reduces startup time when processing many logs with large ZIP files.
- **Mtime-based caching for monitor.json**: The `MonitorJSONBuilder` tracks the modification time (mtime) of each `000.zip` file and caches the extracted `log.v3.json` data and `has_issuers` flag. Before opening a zip file during refresh, the implementation checks the file's current mtime. If the mtime matches the cached mtime, cached data is used without opening the ZIP file, avoiding unnecessary ZIP reads for unchanged files. This optimization significantly reduces disk I/O and CPU usage during periodic refreshes when most archive files remain unchanged, which is typical for large, stable archive sets. Cache entries are automatically cleaned up when logs are removed from the archive index to prevent memory leaks.
...
- **Debug logging for startup**: When `-d/--debug` is enabled, emit detailed debug logs during startup phases (archive discovery, monitor.json snapshot building, HTTP listener establishment) to aid in diagnosing startup stalling issues and performance bottlenecks.
