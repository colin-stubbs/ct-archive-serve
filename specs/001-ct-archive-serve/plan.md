...
- **`/logs.v3.json` URL formation**: Derive the “public base URL” from incoming request headers. Use `Host` header by default. If `CT_HTTP_TRUSTED_SOURCES` is set (CSV of IP addresses/CIDR ranges), trust `X-Forwarded-Host`/`X-Forwarded-Proto` only when the request source IP matches a trusted source; otherwise, log but ignore these headers. For comma-separated `X-Forwarded-*` values, use the first non-empty element after trimming ASCII whitespace. Lowercase the scheme for URL construction. The server does not validate or require a configured hostname/transport.
- **`/logs.v3.json` output**: Must be deterministic with `tiled_logs` sorted by `<log>` ascending per `spec.md` `FR-006`.
- **`/logs.v3.json` refresh failure**: If the most recent refresh attempt fails, `GET /logs.v3.json` returns `503` (temporarily unavailable) until the next successful refresh per `spec.md` `FR-006`.
- **Refresh concurrency protection**: Both `LogListV3JSONBuilder` and `ArchiveIndex` use mutex protection to serialize refresh operations, preventing concurrent refreshes that could waste resources (e.g., opening the same ZIP files multiple times, scanning disk concurrently).
- **ZIP optimization for logs.v3.json**: When building logs.v3.json snapshots, each `000.zip` is opened only once per log to extract both `log.v3.json` and check for `issuer/` entries, rather than opening the same ZIP file twice. This significantly reduces startup time when processing many logs with large ZIP files.
- **Mtime-based caching for logs.v3.json**: The `LogListV3JSONBuilder` tracks the modification time (mtime) of each `000.zip` file and caches the extracted `log.v3.json` data and `has_issuers` flag. Before opening a zip file during refresh, the implementation checks the file's current mtime. If the mtime matches the cached mtime, cached data is used without opening the ZIP file, avoiding unnecessary ZIP reads for unchanged files. This optimization significantly reduces disk I/O and CPU usage during periodic refreshes when most archive files remain unchanged, which is typical for large, stable archive sets. Cache entries are automatically cleaned up when logs are removed from the archive index to prevent memory leaks.
...
- **Debug logging for startup**: When `-d/--debug` is enabled, emit detailed debug logs during startup phases (archive discovery, logs.v3.json snapshot building, HTTP listener establishment) to aid in diagnosing startup stalling issues and performance bottlenecks.
...
- **Low-cardinality metrics**: expose Prometheus metrics only for `/logs.v3.json` and per-`<log>` aggregates (no per-path, per-endpoint, or per-status labeling).

### Core components (internal/ct-archive-serve)

- **ArchiveIndex**: in-memory index mapping `<log>` → archive folder path + known zip parts, refreshed periodically (`CT_ARCHIVE_REFRESH_INTERVAL`). Refresh operations are protected by a mutex to prevent concurrent disk scans.
- **LogListV3JSONBuilder**: Builds and refreshes `/logs.v3.json` snapshots by extracting `log.v3.json` and checking for `issuer/` entries from `000.zip` files. Refresh operations are protected by a mutex to prevent concurrent refreshes. When building snapshots, each `000.zip` is opened only once per log to perform both operations, optimizing startup time for large archives. Uses mtime-based caching to avoid re-reading unchanged zip files during periodic refreshes: before opening a `000.zip`, the implementation checks the file's modification time and uses cached data if the mtime is unchanged. Cache entries are automatically cleaned up when logs are removed from the archive index.
